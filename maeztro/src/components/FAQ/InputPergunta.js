...
const InputPergunta = ({
  ...
  async function handleClick(event) {
    ...
    let prior = document.querySelector('#formPergunta select').value;
    // console.log(prior);
    ...
    if (textPergunta !== '') {
      ...
      try {
        const response = await fetch(`http://localhost:3004/faqs`, {
          method: 'POST',
          headers: {
        ...
        // console.log(data);
        timeoutRef.current = setTimeout(getFaqs(), 2000);
